...
#Region "Initialization"
    ...
    Private Sub FillControls()
        ...
        ' Numerators
        unitSet = unitsControllerSave.units.UnitNumerators
        For i = 0 To Math.Min(maxControlIndex, unitSet.Count - 1)
            FillControl(unitSet, i, p_isNumerator:=True)
        Next

        ' Denominators
        unitSet = unitsControllerSave.units.UnitDenominators
        For i = 0 To Math.Min(maxControlIndex, unitSet.Count - 1)
...
#Region "Form Controls"
    ...
    Private Sub radBtnCustomType_Checked(sender As Object, e As RoutedEventArgs) Handles radBtnCustomType.Checked
        ...
        If unitsController.units.UnitDenominators.Count > 0 Then btnRemoveSchemaDenominator.IsEnabled = True
    End Sub
...
#Region "Methods: Private"
    ...
    ''' </summary>
    ''' <param name="unitIsNumerator">If true, then the new lists to be generated based on state are for the Numerator controls. Else, it is for the denominator controls.</param>
    ''' <remarks></remarks>
    ...
    ''' <param name="unitIsNumerator">If true, the index is for the Unit object in the Numerator position.</param>
    ''' <remarks></remarks>
    ...
    ''' </summary>
    ''' <param name="unitIsNumerator">IF true, returns the Numerator units. Else, returns the denominator units.</param>
    ''' <returns></returns>
    ''' <remarks></remarks>
    Private Function GetUnitsCollection(ByVal p_isNumerator As Boolean) As List(Of cUnit)
        If p_isNumerator Then
            Return unitsController.units.UnitNumerators
        Else
            Return unitsController.units.UnitDenominators
        End If
    End Function
