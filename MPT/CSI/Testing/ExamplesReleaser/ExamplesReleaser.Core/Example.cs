...
namespace Csi.Testing.ExamplesReleaser.Core
{
    ...
    public class Example : IEquatable<Example>
    {
        ...
        {
            return PathModelFile.Split('\\').Last();
        }
        ...
        private void getExampleData(XContainer root)
        {
            ...
            bool isCSiBridgeCase = (string.Compare(Application, "CSiBridge", StringComparison.OrdinalIgnoreCase) == 0);
            if (isCSiBridgeCase)
            {
                // Grab the CSiBridge file version
                modelPath = modelPath.Replace(".sdb", ".bdb");
            }
            PathModelFile = Path.Combine(pathExample, modelPath);
            
            // Check if example is valid by presence of the CSiBridge file
            if (isCSiBridgeCase && !File.Exists(PathModelFile))
            {
                IsMatchingProgram = false;
                return;
            }
        ...
        {
            return string.IsNullOrEmpty(PathModelFile) ? base.ToString() : ModelFileName();
        }
    }
}
