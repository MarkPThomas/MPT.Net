...
{
    internal static class ReadLinkProperties
    {
        ...
        private static void setLINK_PROPERTY_DEFINITIONS_01_GENERAL(Model model, List<Dictionary<string, string>> table)
        {
            foreach (Dictionary<string, string> tableRow in table)
            {
                ...
                if (tableRow.ContainsKey("Notes")) link.Section.Notes = tableRow["Notes"];
                link.Section.ColorName = tableRow["Color"];
                link.Section.Mass = Adaptor.toDouble(tableRow["Mass"]);
                link.Section.Weight = Adaptor.toDouble(tableRow["Weight"]);
                link.Section.RotationalInertia1 = Adaptor.toDouble(tableRow["RotInert1"]);
                link.Section.RotationalInertia2 = Adaptor.toDouble(tableRow["RotInert2"]);
                link.Section.RotationalInertia3 = Adaptor.toDouble(tableRow["RotInert3"]);
                link.Section.DefinedLength = Adaptor.toDouble(tableRow["DefLength"]);
                link.Section.DefinedArea = Adaptor.toDouble(tableRow["DefArea"]);
                link.Section.PDeltaM2I = Adaptor.toDouble(tableRow["PDM2I"]);
                link.Section.PDeltaM2J = Adaptor.toDouble(tableRow["PDM2J"]);
                link.Section.PDeltaM3I = Adaptor.toDouble(tableRow["PDM3I"]);
                link.Section.PDeltaM3J = Adaptor.toDouble(tableRow["PDM3J"]);
                link.Section.LinkType = Enums.EnumLibrary.ConvertStringToEnumByDescription<eLinkPropertyType>(tableRow["LinkType"]);
