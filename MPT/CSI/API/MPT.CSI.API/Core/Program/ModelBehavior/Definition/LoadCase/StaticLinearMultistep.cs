...
// ***********************************************************************
// <copyright file="StaticLinearMultistep.cs" company="">
//     Copyright ©  2017
// </copyright>
// <summary></summary>
// ***********************************************************************
...
{
    /// <summary>
    /// Represents the static linear multistep load case in the application.
    /// </summary>
    /// <seealso cref="MPT.CSI.API.Core.Program.ModelBehavior.Definition.LoadCase.IStaticLinear" />
    ...
    public class StaticLinearMultistep : CSiApiBase, IStaticLinear
    {
        ...
        {
            _callCode = _sapModel.LoadCases.StaticLinearMultistep.SetCase(name);
        ...
        {
            _callCode = _sapModel.LoadCases.StaticLinearMultistep.GetInitialCase(name, ref initialCase);
            if (throwCurrentApiException(_callCode)) { throw new CSiException(API_DEFAULT_ERROR_CODE); }
        }
        ...
        public void SetInitialCase(string name,
            string initialCase)
        {
            _callCode = _sapModel.LoadCases.StaticLinearMultistep.SetInitialCase(name, initialCase);
            if (throwCurrentApiException(_callCode)) { throw new CSiException(API_DEFAULT_ERROR_CODE); }
        }
            ...
            ref double[] scaleFactor)
        {
            ...
            _callCode = _sapModel.LoadCases.StaticLinearMultistep.GetLoads(name, ref _numberOfItems, ref csiLoadTypes, ref loadNames,  ref scaleFactor);
            if (throwCurrentApiException(_callCode)) { throw new CSiException(API_DEFAULT_ERROR_CODE); }
            ...
            double[] scaleFactor)
        {
            ...
            _callCode = _sapModel.LoadCases.StaticLinearMultistep.SetLoads(name, loadTypes.Length, ref csiLoadTypes, ref loadNames, ref scaleFactor);
            if (throwCurrentApiException(_callCode)) { throw new CSiException(API_DEFAULT_ERROR_CODE); }
        }
#endregion
    }
