...
namespace MPT.CSI.API.Core.Program.ModelBehavior.Design
{
    ...
    public class DesignConcrete : CSiApiBase, IDesignConcrete
    {
        ...
        public void GetSummaryResultsBeam(string name, 
            ...
            out double[] VMajorArea,
            out string[] TLCombo,
            out double[] TLArea,
            out string[] TTCombo,
            out double[] TTArea,
            out string[] errorSummary,
            out string[] warningSummary,
            eItemType itemType = eItemType.Object)
        {
            frameName = new string[0];
            location = new double[0];
            topCombo = new string[0];
            topArea = new double[0];
            botCombo = new string[0];
            botArea = new double[0];
            VMajorCombo = new string[0];
            VMajorArea = new double[0];
            TLCombo = new string[0];
            TLArea = new double[0];
            TTCombo = new string[0];
            TTArea = new double[0];
            errorSummary = new string[0];
            warningSummary = new string[0];

            _callCode = _sapModel.DesignConcrete.GetSummaryResultsBeam(name, ref _numberOfItems, 
        ...
        /// <param name="name">The name of an existing frame object or group, depending on the value of the ItemType item.</param>
        /// <param name="frameName">This is an array that includes each frame object name for which results are obtained.</param>
        /// <param name="designOption">This is an array that includes the design option for each frame object.</param>
        /// <param name="location">This is an array that includes the distance from the I-end of the frame object to the location where the results are reported. [L]</param>
            ...
            eItemType itemType = eItemType.Object)
        {
            ...
            PMMArea = new double[0];
            PMMRatio = new double[0];
            VMajorCombo = new string[0];
            AVMajor = new double[0];
            VMinorCombo = new string[0];
            AVMinor = new double[0];
            errorSummary = new string[0];
            warningSummary = new string[0];

            int[] myOption = new int[0];

            _callCode = _sapModel.DesignConcrete.GetSummaryResultsColumn(name, ref _numberOfItems, 
            ...
            designOption = new eDesignType[myOption.Length];
            for (int i = 0; i < myOption.Length; i++)
            {
                designOption[i] = (eDesignType) myOption[i];
            }
        }
        ...
        public void GetSummaryResultsJoint(string name,
            out string[] frameName,
            out string[] LCJSRatioMajor,
            out double[] JSRatioMajor,
            out string[] LCJSRatioMinor,
            out double[] JSRatioMinor,
            out string[] LCBCCRatioMajor,
            out double[] BCCRatioMajor,
            out string[] LCBCCRatioMinor,
            out double[] BCCRatioMinor,
            out string[] errorSummary,
            out string[] warningSummary,
            eItemType itemType = eItemType.Object)
        {
            frameName = new string[0];
            LCJSRatioMajor = new string[0];
            JSRatioMajor = new double[0];
            LCJSRatioMinor = new string[0];
            JSRatioMinor = new double[0];
            LCBCCRatioMajor = new string[0];
            BCCRatioMajor = new double[0];
            LCBCCRatioMinor = new string[0];
            BCCRatioMinor = new double[0];
            errorSummary = new string[0];
            warningSummary = new string[0];

            _callCode = _sapModel.DesignConcrete.GetSummaryResultsJoint(name, 
                            ref _numberOfItems, ref frameName, 
