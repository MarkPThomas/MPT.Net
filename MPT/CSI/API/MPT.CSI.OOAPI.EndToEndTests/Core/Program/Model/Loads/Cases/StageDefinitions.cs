...
namespace MPT.CSI.OOAPI.Core.Program.Model.Loads.Cases
{
    ...
    public class StageDefinitions
    {
        ...
        #region Output
        /// <summary>
        /// The name of each stage definition assigned to the load case.
        /// </summary>
        /// <returns>System.String[].</returns>
        public string[] ToArrayNames()
        {
            string[] arrayNames = new string[_items.Count];

            for (int i = 0; i < _items.Count; i++)
            {
                arrayNames[i] = _items[i].NameOutput;
            }
        ...
        /// <summary>
        /// The duration (in days) of each stage definition assigned to the load case.
        /// </summary>
        /// <returns>System.String[].</returns>
        public double[] ToArrayDuration()
        {
            double[] arrayDuration = new double[_items.Count];

            for (int i = 0; i < _items.Count; i++)
            {
                arrayDuration[i] = _items[i].Duration;
            }
        ...
        /// <summary>
        /// The comment associated with each stage definition assigned to the load case.
        /// </summary>
        /// <returns>System.String[].</returns>
        public string[] ToArrayComment()
        {
            string[] arrayComments = new string[_items.Count];

            for (int i = 0; i < _items.Count; i++)
            {
                arrayComments[i] = _items[i].Comment;
            }
        ...
        /// <summary>
        /// The status of saving output for each stage definition assigned to the load case.
        /// </summary>
        /// <returns>System.String[].</returns>
        public bool[] ToArrayIsSaved()
        {
            bool[] arrayToBeSaved = new bool[_items.Count];

            for (int i = 0; i < _items.Count; i++)
            {
                arrayToBeSaved[i] = _items[i].OutputIsToBeSaved;
            }
        ...
        {
            _staticNonlinearStaged?.SetStageDefinitions(CaseName,
                ToArrayDuration(),
                ToArrayIsSaved(),
                ToArrayNames(),
                ToArrayComment());
