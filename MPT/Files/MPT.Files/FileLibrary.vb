...
#Region "Querying"
    ''' <summary>
    ''' Gets the file date for when the file was last modified.
    ''' </summary>
    ''' <param name="filePath">The file path.</param>
    ''' <returns>System.String.</returns>
    Public Shared Function GetFileDateModified(ByVal filePath As String) As String
        Return File.GetLastWriteTime(filePath).ToShortDateString()
    End Function
#End Region
...
#Region "Create File"
    ...
    ''' <param name="deleteExisting">True: The any existing file at the same path will be deleted before writing a new file. 
    ''' False: The content will be appended to the existing file.</param>
    ''' <remarks></remarks>
    Public Shared Sub WriteTextFile(ByVal filePath As String,
                                    ByVal content As String,
                                    ByVal deleteExisting As Boolean)
        If deleteExisting Then ComponentDeleteFileAction(filePath)

        Using objWriter As New StreamWriter(filePath, append:=True)
...
#Region "Processes/Programs"
    ...
    ''' Note that this only works accurately if there is a single instance of the process running.
    ''' Otherwise, this will always return true if at least one instance is running.
    ''' </summary>
    ...
    Public Shared Function IsProcessRunning(ByVal processName As String) As Boolean
        ...
        Catch ex As Exception
            RaiseEvent Log(New LoggerEventArgs(ex, NameOfParam(Function() processName), processName))
            Return False
        End Try
    End Function
    ...
    ''' Note that this only works accurately if there is a single instance of the process running. 
    ''' Otherwise, only the first process in the list is checked.
    ''' </summary>
    ''' <param name="processName">Name of the process to check. It does not matter if ".exe" is included.</param>
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function IsProcessResponding(ByVal processName As String) As Boolean
        Try
            Dim processes() As Process = Process.GetProcessesByName(processName)

            ' Tests the Responding property for a True return value. 
            Return (processes.Count > 0 AndAlso processes(0).Responding) 
        Catch ex As Exception
            RaiseEvent Log(New LoggerEventArgs(ex, NameOfParam(Function() processName), processName))
            Return False
        End Try
    ...
    ''' Note that this only works accurately if there is a single instance of the process running. 
    ''' Otherwise, all processes of the provided name will be closed.
    ''' </summary>
    ''' <param name="processName">Name of the process to check. It does not matter if ".exe" is included.</param>
    ''' <remarks></remarks>
    Public Shared Sub EndProcess(ByVal processName As String)
        Try
            Dim processes() As Process = Process.GetProcessesByName(processName)
